...
合理的使用Java多线程可以更好地利用服务器资源。一般来讲，线程内部有自己私有的线程上下文，互不干扰。但是当我们需要多个线程之间相互协作的时候，就需要我们掌握Java线程的通信方式。下面将介绍Java线程之间的几种通信原理。
...
**总结**：锁：同一时间只有一个线程持有，可以拿结婚和离婚来理解

​           同步：线程同步是线程之间按照**一定的顺序**执行
...
**总结**： 等待（wait）/通知(notify/notifyAll)  ,必须针对同一个对象锁，notify注意并没有释放锁动作
...
**总结**：ThreadLocal就是一个线程私有变量，为每个线程创建一个副本
